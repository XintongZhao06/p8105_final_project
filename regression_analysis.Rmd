...
Our response variable is **completion_rate**, which exhibits neither strong skewness nor heavy tails, as shown in the [EDA section](topic1.html#completion-dist). Therefore, no additional transformation or preprocessing of the outcome variable is needed before fitting the regression models.
...
The three linear models show small but incremental improvements in fit as additional predictors are added. The baseline model has an R-squared of `r round(summary(model_base)$r.squared, 4)`, 
while the engagement model increases it slightly to `r round(summary(model_add)$r.squared, 4)`. 
The stepwise model achieves the highest R-squared at `r round(summary(step_back)$r.squared, 4)`.
...
[Next: Trend Prediction →](topic2.html)